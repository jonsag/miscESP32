 ...
 *  more feedback, new controls and other tweaks and changes,
 ...
 * an access point called "ESP32-CAM-CONNECT" (password: "InsecurePassword")
 *
 */
...
// Start with access point mode disabled, wifi setup will activate it if
...
void setup()
{
    ...
    // Info line; use for Info messages; eg 'This is a Beta!' warnings, etc. as necessary
    // Serial.print("\r\nThis is the 4.1 beta\r\n");

    // As a final init step chomp out the serial buffer in case we have received mis-keys or garbage during startup
    while (Serial.available())
        Serial.read();
}
...
void loop()
{
    ...
    if (accesspoint)
    {
        // Access point is permanently up, so just loop, servicing the captive portal as needed
